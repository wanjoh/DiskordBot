...
async def pusti(ctx, *, url):
    ...
    if ctx.author.voice:
        ...
        voice.play(discord.FFmpegPCMAudio(s_songName), holup(voice, discord.FFmpegPCMAudio(s_songName)))
        while loop > 0:
            holup(voice)
            voice.play(discord.FFmpegPCMAudio(s_songName))
            loop -= 1
        loop = 0
